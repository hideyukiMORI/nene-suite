...
use NeNeSuite\Tenancy\EnableOrganizationHandler;
...
final class EnableOrganizationHandlerTest extends TestCase
{
    ...
    public function testEnablesForSuperadmin(): void
    {
        $response = $this->handler($this->seeded())->handle(
            $this->request('POST', '/api/v1/organizations/' . self::ORG_ID . '/enable', $this->superadminToken(), [], ['id' => self::ORG_ID]),
        );

        self::assertSame(200, $response->getStatusCode());
        self::assertSame('active', $this->decode($response)['status']);
    }

    public function testIsIdempotentWhenAlreadyActive(): void
    {
        $handler = $this->handler($this->seeded());
        $handler->handle($this->request('POST', '/api/v1/organizations/' . self::ORG_ID . '/enable', $this->superadminToken(), [], ['id' => self::ORG_ID]));

        $response = $handler->handle($this->request('POST', '/api/v1/organizations/' . self::ORG_ID . '/enable', $this->superadminToken(), [], ['id' => self::ORG_ID]));
    ...
    public function testRejectsNonSuperadminWithForbidden(): void
    {
        ...
        $this->handler($this->seeded())->handle(
            $this->request('POST', '/api/v1/organizations/' . self::ORG_ID . '/enable', $this->nonSuperadminToken(), [], ['id' => self::ORG_ID]),
        );
    }
    ...
    public function testRejectsUnauthenticated(): void
    {
        ...
        $this->handler($this->seeded())->handle(
            $this->request('POST', '/api/v1/organizations/' . self::ORG_ID . '/enable', null, [], ['id' => self::ORG_ID]),
        );
    }
    ...
    public function testRejectsUnknownIdWithNotFound(): void
    {
        ...
        $this->handler()->handle(
            $this->request('POST', '/api/v1/organizations/' . self::UNKNOWN_ID . '/enable', $this->superadminToken(), [], ['id' => self::UNKNOWN_ID]),
        );
    }
    ...
    private function seeded(): InMemoryOrganizationRepository
    {
        ...
        $organizations->save(new Organization(self::ORG_ID, self::ORG_EXT, 'Acme', 'acme', OrganizationStatus::Disabled, '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z'));

        return $organizations;
    }

    private function handler(?InMemoryOrganizationRepository $organizations = null): EnableOrganizationHandler
    {
        return new EnableOrganizationHandler(
            $this->guard(),
            new EnableOrganizationUseCase(
